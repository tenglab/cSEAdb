...
bw_path <- system.file("extdata","dummy_bw_path",package="cSEAdb")
bigWigs  <- read.table(bw_path,sep="\t",header=F)
head(bigWigs)
```

```{r create plot dataframe}
...
```{r create plot dataframe}
names_gr <- names(bw_gr$bw_gr)
for (i in 1:length(names_gr)) {
  export.bw(bw_gr$bw_gr[[i]],paste0("../inst/extdata/",names_gr[i],"_dummy.bw"))
}
```
