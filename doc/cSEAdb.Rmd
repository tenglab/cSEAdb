...
graphics: yes
vignette: >
  %\VignetteIndexEntry{cSEAdb user's guide}
  %\VignettePackage{cSEAdb}
...
# bed file
hits_2 <- search_db("../data/se_query_example.bed",
                     query_type="se_bed",se_db=cSEAdb)
...
hits_cancer <- search_db(query=c("Lung non small cell carcinoma"),
...
bw_download_link <- read.table("../data/bw_all_plinks.txt",sep="\t",header=F)
head(bw_download_link)

# Create a bigwig list with the path of bigwig files
# Assume the path of downloaded bigwig files are in file "bw_path.txt"
...
usr_bigwig <- "../data/example_signal.bw"
...
# Create plot object for gviz package
...
This section demonstrates how to generate chromosome maps with the information extracted from _cSEAdb_. Here we use A549 as an example.
...
cell_spec <- readRDS("../data/cell_spec_for_chr_plot.rds")
